...
class MinecraftLauncher:
    def init_win(self):
        os.chdir("D:\\desktop\\minecraft luncher\\source code\\version 3.4")
        # 初始化窗口
        self.mclist = []
        self.root = tk.Tk()
        ...
        def load_mclist():
            try:
                mclist_tmp = requests.get("https://bmclapi2.bangbang93.com/mc/game/version_manifest_v2.json").json()[
                    "versions"]
            except requests.RequestException:
                mclist_tmp = mclib.utils.get_version_list()
            for i in mclist_tmp:
                self.mclist.append(i["id"])

        image_dh = tk.PhotoImage(file=os.path.abspath("./PML/PixelarticonsPixelarticons.png"))
    ...
    def __init__(self):
        ...
        self.PML = tk.Tk()
        # 读取opinion.txt中的信息
        with open(os.path.abspath("./PML/opinions.txt"), encoding="gbk") as f:
            tmp = f.read().split("\n")
            for i in tmp:
                match i:
        ...
        self.PML.iconbitmap(os.path.abspath("./PML/ddd.ico"))
        # 初始化其他变量和控件
        ...
        self.minecraft_version_list = os.listdir(os.path.abspath(self.minecraft_dir_path + r"\versions"))
        ...
        self.label = tk.Label(self.PML, text="下载状态:", bd=1, relief=tk.SUNKEN, anchor=tk.W)
        ...
        self.version_list.bind("<Double-Button-1>", self.download_threading_start)
        ...
        # 设置
        self.Set_PML_Theme = ttk.Button(self.PML_Settings, text="导入PML配置文件", command=self.importthemepack)
        self.Set_PML_Theme.grid(row=1, column=1)
        ...
        self.advanced_settings.grid(row=2, column=1)
        self.banben_geli = tk.Checkbutton(self.PML_Settings, text="打开版本隔离", variable=self.banben_geli_var,
                                          command=self.ed_banben_geli)
        self.banben_geli.grid(row=3, column=1)
        if self.advanced_settings_var.get() == 1:
            self.butt = ttk.Button(self.PML_Settings, text="打开Pixel Server Launcher(实验性)", command=ServerLauncher)
            self.butt.grid(row=2, column=2, columnspan=2)
        self.themelist = ttkthemes.THEMES
        self.tl = ttk.Combobox(self.PML_Settings, values=list(self.themelist))
        self.tl.grid(row=1, column=2)
        self.butt2 = ttk.Button(self.PML_Settings, command=self.savethethemepack, text="导出PML主题文件")
        self.butt2.grid(row=1, column=3)
        self.butt3 = ttk.Button(self.PML_Settings, command=self.apply, text="应用主题")
        self.butt3.grid(row=1, column=4)

        # 启动
        tmp2 = ttk.Notebook(self.PML_Menu, width=100000, height=1000000)
        self.offline = tk.Frame(self.PML_Menu)
        self.offline.pack()
        self.online = tk.Frame(self.PML_Menu)
        self.online.pack()
        photo = tk.PhotoImage(file=os.path.abspath("./PML/PixelarticonsPixelarticons.png"))
        self.steve = ttk.Label(self.PML_Menu, image=photo)
        self.steve.pack(pady=50)
        ttk.Label(self.PML_Menu, text="欢迎来到Pixel Minecraft Launcher v3.4!", font=("宋体", 20)).pack(pady=20)
        style = ttk.Style()
        style.configure('RoundedButton.TButton',
                        foreground='black',
                        borderwidth=0,
                        relief=tk.RAISED)
        self.l1 = ttk.Label(self.offline, text="↓---输入用户名---↓")
        self.l1.pack()
        self.username = ttk.Combobox(self.offline, width=22, values=self.namelist)
        self.username.pack()
        ...
        self.start = tk.Button(self.offline, textvariable=self.version_start_var, command=self.threading_starting,
                               width=30, anchor="center", borderwidth=1, relief="flat")
        self.changever = tk.Button(self.offline, text="版本选择", command=self.select_version_0,
                                   width=30, anchor="center", borderwidth=1, relief="flat")
        self.start.pack()
        tk.Label().pack()
        self.changever.pack()
        tmp2.add(self.offline, text="--离线账号--")

        tk.Label(self.online, text="正在制作中").pack()

        tmp2.add(self.online, text="--正版账号--")
        tmp2.pack()

        # 主事件循环
        self.PML.mainloop()
        os.chdir(self.workon_dictionary)
        with open(os.path.abspath("./PML/opinions.txt"), "w+") as f:
    ...
    def apply(self):
        try:
            self.PML_style.set_theme(self.tl.get())
        except:
    ...
    def savethethemepack(self):
        theme = self.PML_style.get_themes()
        title = self.PML.title()
        savepath = fdl.asksaveasfilename(defaultextension="*.Ptp", filetypes=[("PML主题文件", "*.Ptp")])
    ...
    def importthemepack(self):
        filepath = fdl.askopenfilename(filetypes=[("PML主题文件", "*.Ptp")])
    ...
    def select_version_0(self):
        ...
        self.win.iconbitmap(os.path.abspath("./PML/ddd.ico"))
        ...
        self.minecraft_version_list = os.listdir(os.path.abspath(f"{self.minecraft_dir_path}/versions"))
        self.vl.delete(0, "end")
        for i in self.minecraft_version_list:
            self.vl.insert(tk.END, i)
    ...
    def set_status(self, status: str):
        # self.label.config(text=f"下载状态：{status}")
        # self.label.update()
        print(f"下载状态：{status}")

    def set_progress(self, progress: int):
        if self.current_max != 0:
            # self.label.config(text=f"下载状态:{progress}/{self.current_max}")
            ...
            if self.selected_version_to_start in self.minecraft_version_list:
                if self.banbengeli == 1:
                    ...
                    options = {
                        ...
                        "-Dminecraft.launcher.version": "3.2",
    ...
    def download_threading_start(self, *args):
...
class ServerLauncher:
    def __init__(self):
        self.workon_dictionary = os.getcwd()
        self.win = tk.Tk()
        with open(os.path.abspath("./PML/PSLopinions.txt")) as pslo:
    ...
    def open_server(self):
        ...
        if serverpath != "":
            if os.path.exists(os.path.join(serverpath, "PSLopinion.txt")):
                opinion = open(os.path.abspath(os.path.join(serverpath, "PSLopinion.txt"))).read().split("\n")
    ...
    def add_new_minecraft_server(self):
        ...
        self.win_l.iconbitmap(os.path.abspath("./PML/ddd.ico"))
    ...
    def next(self):
        ...
        self.win_p.iconbitmap(os.path.abspath("./PML/ddd.ico"))
    ...
    def next_2(self):
        ...
        self.win_f.iconbitmap(os.path.abspath("./PML/ddd.ico"))
    ...
    def PSL_Settings(self):
        ...
        self.root.iconbitmap(os.path.abspath("./PML/ddd.ico"))
        ...
        os.chdir(self.workon_dictionary)
        with open(os.path.abspath("./PML/PSLopinions.txt"), "w") as pslo:
